...
class ImageRotator:
    ...
    def __call__(self, x_list, pivot, reverse=False):
        ...
        for i, angle in enumerate(self.angles):
            x = x_list[i].unsqueeze(0)  # Add batch dim [1, C, H, W]

            # Rotation angle
            alpha = angle if not reverse else (-1.0 * angle)
            theta = torch.ones(1) * alpha

            # Get image size
            B, C, H, W = x.shape

            # Create rotation matrix (normalized for affine_grid)
            angle_rad = theta * (3.141592653589793) / 180  # ← torch.pi を数値で代用
            cos = torch.cos(angle_rad)
            sin = torch.sin(angle_rad)

            # Create normalized center
            center_x = (pivot[1] / (W - 1)) * 2 - 1
            center_y = (pivot[0] / (H - 1)) * 2 - 1

            # Build the affine matrix manually
            M = torch.zeros(1, 2, 3, device=x.device)
            M[:, 0, 0] = cos
            M[:, 0, 1] = -sin
            M[:, 1, 0] = sin
            M[:, 1, 1] = cos
            M[:, 0, 2] = (1 - cos) * center_x + sin * center_y
            M[:, 1, 2] = (1 - cos) * center_y - sin * center_x

            # Affine grid
            grid = F.affine_grid(M, size=x.size(), align_corners=True)

            # Warp
            x_warped = F.grid_sample(x.float(), grid, align_corners=True, mode='bilinear', padding_mode='zeros')
