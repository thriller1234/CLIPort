"""Miscellaneous utilities.secondal"""
...
class ImageRotator:
    ...
    def __call__(self, x_list, pivot, reverse=False):
        ...
        for i, angle in enumerate(self.angles):
            ...
            B, C, H, W = x.shape  # 入力テンソルサイズ

            # M: [2, 3] or [1, 2, 3]
            if M.dim() == 2:
                M_expand = M.unsqueeze(0).expand(B, -1, -1).to(x.device)
            elif M.dim() == 3 and M.shape[0] == 1:
                M_expand = M.expand(B, -1, -1).to(x.device)
            else:
                M_expand = M.to(x.device)

            # Affine grid 作成
            grid = F.affine_grid(M_expand, size=x.size(), align_corners=False)

            # Warp 実行
            x_warped = F.grid_sample(x.float(), grid, align_corners=False, mode='bilinear', padding_mode='zeros')
            x_warped = x_warped
